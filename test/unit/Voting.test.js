...
describe(`Contract ${CONTRACT_NAME} `, () => {
  ...
  describe("Initialization", () => {
    ...
    it("Should NOT change workflow status if you're not owner", async () => {
      await expectRevert(voting.connect(this.addr1).startProposalsRegistering(),"Ownable: caller is not the owner");
      await expectRevert(voting.connect(this.addr1).endProposalsRegistering(),"Ownable: caller is not the owner");
      await expectRevert(voting.connect(this.addr1).startVotingSession(),"Ownable: caller is not the owner");
      await expectRevert(voting.connect(this.addr1).endVotingSession(),"Ownable: caller is not the owner");
      await expectRevert(voting.connect(this.addr1).tallyVotes(),"Ownable: caller is not the owner");
    });


    it("Should NOT add voter if you're not owner", async () => {
      await expectRevert(voting.connect(this.addr1).addVoter(this.addr1.address),"Ownable: caller is not the owner");
    });
  ...
  describe("Registering Voters", () => {
    ...
    it("Should NOT register voters if already registered", async () => {
      ...
      await expectRevert(voting.addVoter(this.addr1.address),"Already registered");
    });


    it("Should NOT register voters if wrong workflow status", async () => {
      await voting.startProposalsRegistering();
      await expectRevert(voting.addVoter(this.addr1.address),"Voters registration is not open yet");
    });


    it("Should NOT get voter if you're not voter", async () => {
      expectRevert(voting.connect(this.addr1).getVoter(this.owner.address),"You're not a voter");
    });
  });
  // -
  ...
  describe("Proposals Registration Started", () => {
    ...
    it("Should add proposals", async () => {
      for (let index = 1; index < numberProposals; index++) { // Start at 1 because of genesis proposal
      ...
      await expectRevert.unspecified(voting.getOneProposal(numberProposals + 1));
    })


    it("Should NOT add an empty proposal", async () => {
      await expectRevert(voting.addProposal(""),"Vous ne pouvez pas ne rien proposer");
    })
    

    it("Should NOT get proposal if you're not voter", async () => {
      await expectRevert(voting.connect(this.addr2).getOneProposal(0),"You're not a voter");
    })


    it("Should NOT add proposal if you're not voter", async () => {
      await expectRevert(voting.connect(this.addr2).addProposal("Proposal 1"),"You're not a voter");
    })


    it("Should NOT set vote if wrong workflow status", async () => {
      await expectRevert(voting.setVote(0),"Voting session havent started yet")
    })
  });
  // -
  ...
  describe("Proposals Registration Ended", () => {
    ...
    it("Should NOT add proposal if registering not open", async () => {
      await expectRevert(voting.addProposal("Proposal 1"),"Proposals are not allowed yet");
    })
  })
  // -
  ...
  describe("Voting Session Started", () => {
    ...
    it("Should NOT vote if you're not voter", async () => {
      await expectRevert(voting.connect(this.addr2).setVote(0),"You're not a voter");
    })
    ...
    it("Should NOT set vote if proposal doesn't exist", async () => {
      await expectRevert(voting.setVote(proposalsLength + 1), "Proposal not found");
    })
  })
  // -
